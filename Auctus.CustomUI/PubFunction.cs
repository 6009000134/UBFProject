...
namespace Auctus.CustomUI
{
    ...
    public class PubFunction
    {
        ...
        public static string OAModifyService(string jsson)
        {
            //参数1：准入码，参数2：json
            string OACode = PubFunction.GetOACode();
            string result = string.Empty;
            ...
            if (IsTest == "是")//测试环境
            {
                OAWorkflowModify4Test.ModifyWorkflowPortTypeClient service = new OAWorkflowModify4Test.ModifyWorkflowPortTypeClient();
                try
                {
                    result = service.ChangeWorkflow(OACode, jsson);
                    logger.Error("调用OA审批内网测试接口成功，返回的结果为：" + result);
                }
                catch (Exception ex)
                {
            ...
            {
                OAWorkflowModify.ModifyWorkflowPortTypeClient service = new OAWorkflowModify.ModifyWorkflowPortTypeClient();
                try
                {
                    result = service.ChangeWorkflow(OACode, jsson);
                    logger.Error("调用OA审批接口成功，返回的结果为：" + result);
        ...
        /// <param name="OAFlowID"></param>
        /// <returns></returns>
        public static string GetOAFlowScript(string userCode,string OAFlowID)
        {
            string UserCodeBase64 = CommonFunction.Base64Encode(Encoding.UTF8, userCode);
            string OAIP = CommonFunction.GetOAIP();
            string url = "http://" + OAIP + string.Format("/Auctus/jsp/WorkflowSSO.jsp?usercode={0}&requestid={1}", System.Web.HttpUtility.UrlEncode(UserCodeBase64, Encoding.UTF8), OAFlowID);
