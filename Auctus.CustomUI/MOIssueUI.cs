...
namespace Auctus.CustomUI
{
    ...
    /// 生产领退料
    /// </summary>
    public class MOIssueUI : UFSoft.UBF.UI.Custom.ExtendedPartBase
    {
        ...
        public override void AfterRender(IPart Part, EventArgs args)
        {
            ...
            IUIRecord rec = uiPart.Model.Views["IssueDoc"].FocusedRecord;
            if (rec != null && Convert.ToInt64(rec["ID"].ToString()) > 0)
            {
                //btnQuickCreate
                //查询OA的流程ID
                string sql = string.Format(@"SELECT 
a.DescFlexField_PrivateDescSeg5 OAFlowID,b.DescFlexField_PrivateDescSeg1 IsToOA,a.DocState
FROM dbo.MO_IssueDoc a INNER JOIN dbo.MO_IssueDocType b ON a.IssueDocType=b.ID WHERE a.ID={0}", rec["ID"].ToString());
                ...
                if (IsToOA == "1")
                {
                    IUFToolbar bar = (IUFToolbar)Part.GetUFControlByName(Part.TopLevelContainer, "Toolbar2");
        ...
        {
            IUIRecord rec = uiPart.Model.Views["IssueDoc"].FocusedRecord;
            string sql = string.Format(@"SELECT  a.DescFlexField_PrivateDescSeg5 OAFlowID 
FROM dbo.MO_IssueDoc a WHERE a.ID={0}", rec["ID"].ToString());
            DataSet ds = new DataSet();
            string OAFlowID = "";
            DataAccessor.RunSQL(DatabaseManager.GetCurrentConnection(), sql, null, out ds);
            foreach (DataRow row in ds.Tables[0].Rows)
